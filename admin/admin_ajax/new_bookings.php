<?php
    ...
    {  
        ...
        if(mysqli_num_rows($res) == 0) {
            echo "<tr><td colspan='5' class='text-center'><div style='padding: 40px;'><i class='bi bi-inbox' style='font-size: 3rem; color: var(--admin-gold);'></i><h5 style='color: var(--admin-primary); margin-top: 15px;'>No Data Found</h5></div></td></tr>";
            exit;
        }
        ...
        while($data = mysqli_fetch_assoc($res)) {
            ...
            $table_data .= "
                ...
                <td>
                    <b style='color: var(--admin-primary);'>Check in:</b> $checkin
                    <br>
                    <b style='color: var(--admin-primary);'>Check out:</b> $checkout
                    <br>
                    <b style='color: var(--admin-primary);'>Paid:</b> <span style='color: var(--admin-gold); font-weight: 700;'>₹$data[total_pay]</span>
                    <br>
                    <b style='color: var(--admin-primary);'>Date:</b> $date
                </td>

                <td>
                <button type='button' onclick='assign_room($data[booking_id])' class='btn text-white btn-sm fw-bold custom-bg shadow-none mb-2' data-bs-toggle='modal' data-bs-target='#assign-room' style='padding: 8px 15px; border-radius: 6px; width: 100%;'>
                    <i class='bi bi-check2-square me-1'></i>Assign Room
                </button>
                <br>
                <button type='button' onclick='cancel_booking($data[booking_id])' class='btn btn-outline-danger btn-sm fw-bold shadow-none' style='padding: 8px 15px; border-radius: 6px; width: 100%;'>
