        ...
        while($row = mysqli_fetch_assoc($res)) {
            ...
            echo <<< data
                <div class="col-md-2 mb-3">
                    <div class="card text-white" style="background: linear-gradient(135deg, var(--admin-primary) 0%, var(--admin-primary-dark) 100%); border-radius: 12px; overflow: hidden; box-shadow: 0 8px 20px rgba(0,0,0,0.15);">
                        <img src="$path$row[picture]" class="card-img" style="opacity: 0.9; height: 200px; object-fit: cover;">
                        <div class="card-img-overlay d-flex flex-column justify-content-between">
                            <button type="button" onclick="del_team_member($row[sl_no])" class="btn btn-danger btn-sm shadow-none align-self-end" style="border-radius: 6px; font-weight: 600;">
                                <i class="bi bi-trash3"></i>
                            </button>
                            <div></div>
                        </div>
                        <p class="card-text text-center px-3 py-2" style="background: rgba(0,0,0,0.5); margin: 0; font-weight: 600;">$row[name]</p>
                    </div>
                </div>
            data;
        }
