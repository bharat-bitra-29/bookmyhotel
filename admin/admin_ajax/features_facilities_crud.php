        ...
        while($row = mysqli_fetch_assoc($res)) {
            echo <<< data
                <tr style='transition: all 0.3s ease;'>
                    ...
                    <td style='color: var(--text-dark);'>$row[name]</td>
                    <td>
                    <button type="button" onclick="del_feature_data($row[id])" class="btn btn-danger btn-sm shadow-none" style='padding: 8px 15px; border-radius: 6px; font-weight: 600;'>
        ...
        while($row = mysqli_fetch_assoc($res)) {
            echo <<< data
                <tr class='align-middle' style='transition: all 0.3s ease;'>
                    <td style='font-weight: 600; color: var(--admin-primary);'>$i</td>
                    <td>
                        <div style='width: 40px; height: 40px; background: linear-gradient(135deg, var(--admin-gold) 0%, var(--admin-gold-light) 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; padding: 8px;'>
                            <img src="$path$row[icon]" style="width: 100%; height: 100%; object-fit: contain; filter: brightness(0) invert(1);">
                        </div>
                    </td>
                    <td style='color: var(--text-dark); font-weight: 600;'>$row[name]</td>
                    <td style='color: var(--text-dark);'>$row[description]</td>
                    <td>
                    <button type="button" onclick="del_facility_data($row[id])" class="btn btn-danger btn-sm shadow-none" style='padding: 8px 15px; border-radius: 6px; font-weight: 600;'>
                        <i class="bi bi-trash3 me-1"></i>Delete
                    </button>
                    </td>
                </tr>
